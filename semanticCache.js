...
class SemanticCache {
  ...
  async generateEmbedding(text) {
    try {
      ...
      console.log(
        `💾 Embedding cached (${this.embeddingCacheSize}/${this.maxEmbeddingCacheSize})`,
      );
      return embedding;
    } catch (error) {
    ...
    try {
      ...
      if (searchResults.total > 0) {
        ...
        if (similarity >= config.SIMILARITY_THRESHOLD) {
          ...
          // Track metrics
          const metadata = bestMatch.value.metadata
            ? JSON.parse(bestMatch.value.metadata)
            : {};
          ...
          const costSaved =
            (estimatedTokens * 0.15 + estimatedTokens * 0.6) / 1000000;
    ...
    try {
      ...
      if (isHit) {
        ...
        metrics.estimated_cost_saved =
          ...
          config.OPENAI_COST_PER_1K_TOKENS;

        // Update average similarity
        metrics.average_similarity =
          (metrics.average_similarity * (metrics.cache_hits - 1) + similarity) /
          metrics.cache_hits;
      } else {
      ...
      console.log(
        `📊 Cache metrics updated: ${metrics.cache_hits}/${metrics.total_requests} (${(metrics.hit_ratio * 100).toFixed(1)}%)`,
      );
  ...
  async getCacheStats() {
    try {
      ...
      return {
        ...
        memory_saved_mb:
          (metrics.total_tokens_saved * config.TOKEN_ESTIMATION_RATIO) /
          config.MEMORY_EFFICIENCY_FACTOR, // Use config values
      };
    } catch (error) {
