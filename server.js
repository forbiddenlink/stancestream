...
import {
  ...
  generateEnhancedMessageOnly,
  updateStanceBasedOnDebate,
} from "./enhancedAI.js";
import {
  RedisMetricsCollector,
  generateContestAnalytics,
} from "./advancedMetrics.js";
import { createServer } from "http";
import sentimentAnalyzer from "./sentimentAnalysis.js";
import keyMomentsDetector, {
  processDebateEvent,
  getKeyMoments,
  getAllKeyMoments,
} from "./keyMoments.js";
import intelligentAgentSystem, {
  generateIntelligentMessage,
} from "./intelligentAgents.js";
import openAIValidator from "./src/services/openAIValidator.js";
import debateOrchestrator, {
  runLangGraphDebate,
} from "./src/agents/debateOrchestrator.js";
import { getLangfuse, flushLangfuse } from "./src/utils/langfuse.js";
import redisOptimizer, {
  startOptimization,
  getOptimizationMetrics,
} from "./redisOptimizer.js";
import advancedFactChecker, {
  checkFactAdvanced,
  getFactCheckAnalytics,
} from "./advancedFactChecker.js";
import platformMetricsEngine, {
  startContestMetrics,
  getLiveContestMetrics,
} from "./platformMetricsEngine.js";
...
function isAllowedOrigin(origin) {
  ...
  if (process.env.CORS_ALLOWLIST) {
    for (const host of process.env.CORS_ALLOWLIST.split(",")
      .map((s) => s.trim())
      .filter(Boolean)) {
  ...
  try {
    ...
    const isAllowedLocal =
      allowedHosts.includes(url.hostname) && allowedPorts.includes(url.port);
    const isVercelPreview =
      url.protocol === "https:" && url.hostname.endsWith(".vercel.app");
    const isBaseAllowed =
      url.protocol === "https:" && baseAllowedDomains.has(url.hostname);
...
try {
  ...
  if (healthCheck.status === "healthy") {
    ...
    try {
      ...
      if (!cacheInitialized) {
        console.warn(
          "⚠️ Cache system failed to initialize - running without semantic cache",
        );
      ...
      console.warn(
        "⚠️ Cache initialization error (non-blocking):",
        cacheError.message,
      );
      console.warn(
        "⚠️ Server will continue without semantic cache functionality",
      );
      global.cacheStatus = "unavailable";
    }
  }
...
try {
  ...
  console.log(
    "⚠️ Sentiment analyzer failed to initialize, will use fallback mode:",
    sentimentInitError.message,
  );
}
...
console.log(
  "⏸️ Redis optimizer disabled - use API endpoints to enable manually",
);
...
console.log(
  "⏸️ Platform metrics disabled - use Analytics view to enable manually",
);
...
// LangGraph feature flag - set via env var or default to false for backward compatibility
const USE_LANGGRAPH = process.env.USE_LANGGRAPH === "true";
console.log(
  `LangGraph orchestration: ${USE_LANGGRAPH ? "ENABLED" : "DISABLED (use USE_LANGGRAPH=true to enable)"}`,
);

// LangSmith observability - auto-detected from env vars
if (
...
app.get("/api/sentiment/:debateId/:agentId/history", async (req, res) => {
  try {
    ...
    console.log(
      `📊 Fetching sentiment history for ${agentId} in debate ${debateId} (${points} points)`,
    );
    ...
    const history = await sentimentAnalyzer.getConfidenceHistory(
      debateId,
      agentId,
      points,
    );
...
app.post("/api/debate/start", async (req, res) => {
  try {
    ...
    const sanitizedAgents = Array.isArray(agents)
      ? agents.map((agent) => sanitizeInput(agent))
      : ["senatorbot", "reformerbot"];

    // Global cooldown check
    const now = Date.now();
    if (now - lastGlobalDebateStart < DEBATE_START_COOLDOWN) {
      console.log(
        `🚫 Debate start request too soon (${now - lastGlobalDebateStart}ms ago), rejecting`,
      );
      return res.status(429).json({
        error: "Too many requests",
        message: `Please wait ${DEBATE_START_COOLDOWN}ms between debate starts`,
        cooldownRemaining:
          DEBATE_START_COOLDOWN - (now - lastGlobalDebateStart),
      });
    }
    ...
    if (activeDebates.has(uniqueDebateId)) {
      console.log(
        `⚠️ Debate ${uniqueDebateId} is already running, rejecting duplicate start request`,
      );
      return res.status(409).json({
        error: "Debate is already running",
        debateId: uniqueDebateId,
        message:
          "Please wait for the current debate to finish or use a different debate ID",
        currentStatus: activeDebates.get(uniqueDebateId),
      });
    }

    console.log(
      `🎯 Starting debate: ${uniqueDebateId} on topic: ${sanitizedTopic}`,
    );
    ...
    // Choose orchestration method based on feature flag
    const debateRunner = USE_LANGGRAPH
      ? runLangGraphDebateWithBroadcast(
          uniqueDebateId,
          sanitizedAgents,
          sanitizedTopic,
          debateProcess,
        )
      : runDebateRounds(uniqueDebateId, sanitizedAgents, sanitizedTopic);

    debateRunner.finally(() => {
    ...
    res.json({
      ...
      orchestration: USE_LANGGRAPH ? "langgraph" : "legacy",
...
app.post("/api/debate/:id/stop", async (req, res) => {
  try {
    ...
    console.log(
      `✅ Debate ${id} removed from active debates. Remaining: ${activeDebates.size}`,
    );
...
app.get("/api/debate/:id/messages", async (req, res) => {
  try {
    ...
    const messages = await client.xRevRange(`debate:${id}:messages`, "+", "-", {
      COUNT: limit,
    });
...
app.get("/api/agent/:id/memory/:debateId", async (req, res) => {
  try {
    ...
    const memories = await client.xRevRange(
...
app.post("/api/debates/start-multiple", async (req, res) => {
  try {
    ...
    for (const topic of topics) {
      ...
      // Choose orchestration based on feature flag
      const debateRunner = USE_LANGGRAPH
        ? runLangGraphDebateWithBroadcast(
            debateId,
            agents,
...
// LangGraph-specific debate endpoint - always uses LangGraph regardless of feature flag
app.post("/api/debate/langgraph/start", async (req, res) => {
  try {
    const {
      debateId = `langgraph_${Date.now()}`,
      topic = "climate change policy",
      agents = ["senatorbot", "reformerbot"],
      rounds = 5,
    } = req.body;
    ...
    if (activeDebates.has(sanitizedDebateId)) {
      return res.status(409).json({
        error: "Debate is already running",
        debateId: sanitizedDebateId,
      });
    }

    console.log(
      `Starting LangGraph debate: ${sanitizedDebateId} on "${sanitizedTopic}"`,
    );

    // Initialize Langfuse trace for debate lifecycle
    const langfuse = getLangfuse();
    const debateTrace = langfuse?.trace({
      name: "langgraph-debate-lifecycle",
      input: {
        debateId: sanitizedDebateId,
        topic: sanitizedTopic,
        agents: sanitizedAgents,
    ...
    // Mark debate as active
    activeDebates.set(sanitizedDebateId, {
      topic: sanitizedTopic,
      agents: sanitizedAgents,
      startTime: new Date().toISOString(),
      status: "running",
      ...
      factChecks: 0,
      orchestration: "langgraph",
      langfuseTraceId: debateTrace?.id,
    });
    activeDebatesGauge.set(activeDebates.size);
    ...
    // Start LangGraph debate
    const debateProcess = { cancelled: false };
    runningDebateProcesses.set(sanitizedDebateId, debateProcess);
    ...
    runLangGraphDebateWithBroadcast(
      ...
      .then(() => {
        debateTrace?.end({
          output: {
            ...
            status: "completed",
            messageCount:
              activeDebates.get(sanitizedDebateId)?.messageCount || 0,
          },
      ...
      .finally(async () => {
        activeDebates.delete(sanitizedDebateId);
        activeDebatesGauge.set(activeDebates.size);
        runningDebateProcesses.delete(sanitizedDebateId);
        debateMetrics.concurrentDebates = activeDebates.size;
    ...
    res.json({
      ...
      rounds,
      orchestration: "langgraph",
      message: "LangGraph debate started successfully",
      langsmith:
        process.env.LANGCHAIN_TRACING_V2 === "true" ? "enabled" : "disabled",
      langfuse: getLangfuse() ? "enabled" : "disabled",
      activeDebates: activeDebates.size,
    });
  } catch (error) {
    console.error("Error starting LangGraph debate:", error);
    await flushLangfuse();
    res
      .status(500)
...
// Get LangGraph orchestration status
app.get("/api/orchestration/status", (req, res) => {
  res.json({
    defaultOrchestration: USE_LANGGRAPH ? "langgraph" : "legacy",
    langgraphAvailable: true,
    langsmithEnabled: process.env.LANGCHAIN_TRACING_V2 === "true",
    langsmithConfigured: !!process.env.LANGCHAIN_API_KEY,
    activeDebates: activeDebates.size,
    debateOrchestrations: Array.from(activeDebates.entries()).map(
      ([id, data]) => ({
        debateId: id,
        orchestration: data.orchestration || "legacy",
      }),
    ),
    timestamp: new Date().toISOString(),
  });
...
app.get("/api/cache/metrics", async (req, res) => {
  try {
    ...
    try {
      ...
      console.log(
        "⚠️ Error checking metrics, attempting initialization:",
        error,
      );
      metricsExist = null;
    }
    ...
    if (cacheStats) {
      ...
      const { calculateBusinessMetrics, getDashboardMetrics } =
        await import("./costCalculator.js");
      const businessMetrics = calculateBusinessMetrics(cacheStats);
      const dashboardMetrics = getDashboardMetrics(cacheStats);

      console.log(
        `📊 Cache metrics: ${cacheStats.cache_hits}/${cacheStats.total_requests} hits (${cacheStats.hit_ratio.toFixed(1)}%) - $${businessMetrics.current_usage.monthly_savings}/month saved`,
      );
...
app.get("/api/business/summary", async (req, res) => {
  try {
    ...
    const { getCacheStats } = await import("./semanticCache.js");
    const { calculateBusinessMetrics, generateExecutiveSummary } =
      await import("./costCalculator.js");
    ...
    if (cacheStats && cacheStats.total_requests > 0) {
      ...
      console.log(
        `💰 Business impact: $${businessMetrics.current_usage.monthly_savings}/month, ${businessMetrics.current_usage.cache_efficiency} efficiency`,
      );
      ...
      res.json({
        ...
        message:
          "Business metrics will be available after system processes requests",
      });
    }
  } catch (error) {
...
function getEnhancedMetrics() {
  ...
  return {
    ...
    redisModules: {
      json: {
        keysCount: Math.floor(Math.random() * 10) + 5,
        operations: debateMetrics.agentInteractions,
      },
      streams: {
        keysCount: activeDebates.size * 3,
        operations: debateMetrics.messagesGenerated,
      },
      timeseries: {
        keysCount: activeDebates.size * 2,
        operations: debateMetrics.messagesGenerated,
      },
      vector: {
        keysCount: Math.floor(Math.random() * 50) + 20,
        operations: debateMetrics.factChecksPerformed,
      },
    },
...
async function runDebateRounds(debateId, agents, topic, rounds = 5) {
  ...
  console.log(
    `📊 Active debates at start: ${Array.from(activeDebates.keys()).join(", ")}`,
  );
  console.log(
    `🔧 Running processes at start: ${Array.from(runningDebateProcesses.keys()).join(", ")}`,
  );
  ...
  console.log(
    `✅ Debate process confirmed for ${debateId}, proceeding with rounds...`,
  );
  ...
  for (
    ...
    console.log(
      `🔄 Attempt ${attemptedTurn + 1}, Actual Turn ${actualTurn + 1}: agentIndex=${currentAgentIndex}, agentId=${agentId}, lastSpeaker=${lastSpeakerPreCheck}, agents=[${agents.join(", ")}]`,
    );

    try {
      console.log(
        `🗣️ Turn ${actualTurn + 1} (Round ${roundNumber}): ${agentId} speaking about "${topic}" (${debateId})...`,
      );
      ...
      if (timeSinceLastMessage < 2000) {
        // Minimum 2 seconds between messages per agent
        console.log(
          `⚠️ ${agentId} attempted message too soon (${timeSinceLastMessage}ms ago), waiting and retrying...`,
        );
      ...
      if (lastSpeaker === agentId && actualTurn > 0) {
        console.log(
          `⚠️ ${agentId} spoke last, this should not happen with proper alternation. Force switching to next agent.`,
        );
      ...
      try {
        const recentMessages = await client.xRevRange(
        ...
        if (recentMessages.length > 0) {
          ...
          if (lastMessageAgentId === agentId) {
            console.log(
              `🚨 CRITICAL: ${agentId} was the last message sender! Forcing agent switch to prevent consecutive speaking.`,
            );
            currentAgentIndex = (currentAgentIndex + 1) % agents.length;
            currentAgentIndexPerDebate.set(debateId, currentAgentIndex);
            const newAgentId = agents[currentAgentIndex];
            console.log(
              `🔄 Switched from ${agentId} to ${newAgentId} to maintain alternation`,
            );
            continue;
          }
        }
        ...
        console.log(
          `⚠️ Could not check recent messages: ${messageCheckError.message}`,
        );
      }
      ...
      try {
        ...
        const result = await generateEnhancedMessageOnly(
          agentId,
          debateId,
          topic,
        );
        ...
        if (typeof result === "object" && result.message) {
          ...
          var factCheck = result.factCheck || {
            fact: null,
            score: 0,
            confidence: 0,
          };
        ...
        console.log(
          `⚠️ Enhanced AI failed, falling back to standard: ${enhancedError.message}`,
        );

        // Check cancellation before fallback AI call
        if (debateProcess.cancelled) {
          console.log(
            `⏹️ Debate ${debateId} cancelled before fallback AI generation`,
          );
          return;
        }
      ...
      broadcastRedisOperation(
        "streams",
        `debate:${debateId}:messages → new message`,
        {
          agentId,
      ...
      broadcastRedisOperation(
        "streams",
        `agent:${agentId}:memory → strategic note`,
        {
          agentId,
          type: "statement",
        },
      ...
      if (cacheHit) {
        console.log(
          `🎯 Broadcasting cache hit celebration: ${(similarity * 100).toFixed(1)}% similarity, $${costSaved.toFixed(3)} saved`,
        );
        broadcast({
          type: "cache_hit",
          debateId,
      ...
      if (debateProcess.cancelled || !activeDebates.has(debateId)) {
        console.log(
          `⏹️ Debate ${debateId} cancelled before processing message`,
        );
        return;
      }
      ...
      try {
        ...
        console.log(
          `📊 ${agentId} sentiment: ${sentimentResult.sentiment} (${sentimentResult.confidence} confidence, ${sentimentResult.model})`,
        );
      ...
      try {
        const stanceUpdate = await updateStanceBasedOnDebate(
          agentId,
          debateId,
          topic,
        );
        ...
        console.log(
          `📈 ${agentId} stance evolved: ${stanceUpdate.oldStance.toFixed(3)} → ${stanceUpdate.newStance.toFixed(3)}`,
        );
      } catch (stanceError) {
        console.log(
          `⚠️ Stance evolution failed, using fallback: ${stanceError.message}`,
        );
        ...
        try {
          await client.ts.add(
            timesSeriesKey,
            "*",
            parseFloat(stanceData.newStance).toString(),
          );
        } catch (tsError) {
      ...
      try {
        ...
        console.log(
          `🐛 DEBUG: Creating stance object with topic="${topic}" for debateId=${debateId}`,
        );
        ...
        if (keyMomentResult.created) {
          console.log(
            `🔍 KEY MOMENT CREATED: ${keyMomentResult.moment.type} in debate ${debateId}`,
          );
        ...
        console.log(
          `⚠️ Key moment detection failed: ${keyMomentError.message}`,
        );
      }
      ...
      if (debateProcess.cancelled || !activeDebates.has(debateId)) {
        console.log(
          `⏹️ Debate ${debateId} cancelled before broadcasting message`,
        );
        return;
      }
      ...
      console.log(
        `🐛 DEBUG: Broadcasting stance with topic="${topic}" for debateId=${debateId}`,
      );
      broadcast({
        type: "new_message",
        debateId,
      ...
      try {
        ...
        for (const otherAgentId of agents) {
          try {
            const otherProfile = await client.json.get(
              `agent:${otherAgentId}:profile`,
            );
            if (otherProfile && otherProfile.stance) {
              // Map topic to stance key using the utility function
              const { topicToStanceKey } =
                await import("./messageGenerationCore.js");
              const stanceKey = topicToStanceKey(topic);
            ...
            console.log(
              `⚠️ Could not get stance for ${otherAgentId}: ${profileError.message}`,
            );
            currentStances[otherAgentId] = 0; // Neutral fallback
          }
        }
        ...
        console.log(
          `📈 Stance broadcast sent - Turn ${currentTurn} (Round ${currentRound}): SenatorBot(${(currentStances.senatorbot || 0).toFixed(2)}), ReformerBot(${(currentStances.reformerbot || 0).toFixed(2)})`,
        );
      } catch (individualStanceError) {
        console.log(
          `⚠️ Failed to broadcast individual stance update: ${individualStanceError.message}`,
        );
      }
      ...
      if (!activeDebates.has(debateId) || debateProcess.cancelled) {
        console.log(
          `⏹️ Debate ${debateId} was stopped after message generation`,
        );
        return;
      }
      ...
      for (let waited = 0; waited < waitTime; waited += checkInterval) {
        ...
        await new Promise((resolve) =>
          setTimeout(resolve, Math.min(checkInterval, waitTime - waited)),
        );
      }
      ...
      console.log(
        `✅ Turn ${actualTurn} completed by ${agentId}. Next: ${agents[currentAgentIndex]}`,
      );
    } catch (error) {
  ...
  try {
    ...
    for (const agentId of agents) {
      try {
        ...
        console.log(
          `⚠️ Could not get stance for ${agentId}: ${profileError.message}`,
        );
        currentStances[agentId] = 0; // Neutral fallback
      }
    }
    ...
    console.log(
      `📈 Stance broadcast sent - Final: SenatorBot(${(currentStances.senatorbot || 0).toFixed(2)}), ReformerBot(${(currentStances.reformerbot || 0).toFixed(2)})`,
    );
  } catch (stanceError) {
...
/**
 * LangGraph-based debate runner with broadcast integration
 * Uses the new LangGraph multi-agent workflow while maintaining
 * compatibility with the existing WebSocket broadcast system
 */
async function runLangGraphDebateWithBroadcast(
  debateId,
  agents,
  topic,
  debateProcess,
  ...
  console.log(`LangGraph debate starting: ${debateId} on "${topic}"`);
  console.log(`Using LangGraph orchestration for multi-agent workflow`);
  ...
  try {
    ...
    console.log(`No previous messages to clear for debate: ${debateId}`);
  }

  let turnCount = 0;

  // Message callback - broadcasts each message as it's generated
  const onMessage = async (msg) => {
    turnCount++;
    const { agentId, message, timestamp, metadata } = msg;
    ...
    let profile;
    try {
      profile = await client.json.get(`agent:${agentId}:profile`);
    } catch (e) {
      profile = { name: agentId };
    }

    // Update fact check metrics
    if (metadata?.factCheck?.fact) {
      debateMetrics.factChecksPerformed++;
      if (activeDebates.has(debateId)) {
        activeDebates.get(debateId).factChecks++;
      }
    ...
    // Get stance data
    let stanceData = { value: 0.5, change: 0 };
    try {
      const { topicToStanceKey } = await import("./messageGenerationCore.js");
      const stanceKey = topicToStanceKey(topic);
      if (profile?.stance?.[stanceKey]) {
        stanceData.value = profile.stance[stanceKey];
      }
    } catch (e) {
      console.log(`Could not get stance for ${agentId}`);
    }
    ...
    broadcast({
      ...
      debateId,
      agentId,
      agentName: profile?.name || agentId,
      message,
      timestamp,
      factCheck: metadata?.factCheck?.fact
        ? {
            fact: metadata.factCheck.fact,
            confidence: metadata.factCheck.confidence,
          }
        : null,
      ...
      aiMetadata: {
        emotionalState: metadata?.emotionalState || "analytical",
        orchestration: "langgraph",
        turnNumber: metadata?.turnNumber || turnCount,
      },
    });
    ...
    try {
      ...
      for (const otherAgentId of agents) {
        try {
          ...
          const stanceKey = topicToStanceKey(topic);
          const stanceValue = otherProfile?.stance?.[stanceKey] || 0.5;
          currentStances[otherAgentId] = (stanceValue - 0.5) * 2;
        } catch (e) {
          currentStances[otherAgentId] = 0;
        }
      }

      const currentRound = Math.floor(turnCount / agents.length) + 1;
      broadcast({
        type: "debate:stance_update",
        debateId,
        senatorbot: currentStances.senatorbot || 0,
      ...
      console.log(`Failed to broadcast stance update: ${stanceError.message}`);
    }

    console.log(`[LangGraph] ${agentId}: ${message.substring(0, 50)}...`);
  ...
  try {
    ...
    if (activeDebates.has(debateId)) {
      console.log(`LangGraph debate ${debateId} completed successfully`);
    ...
    console.log(
      `LangGraph debate completed: ${debateId} with ${messages.length} messages`,
    );
    return messages;
  } catch (error) {
    console.error(`LangGraph debate error: ${error.message}`);
    broadcast({
      type: "error",
      debateId,
      message: `Debate error: ${error.message}`,
      orchestration: "langgraph",
      timestamp: new Date().toISOString(),
    });
    throw error;
...
app.get("/api/contest/live-metrics", async (req, res) => {
  try {
    ...
    const contestMetrics = {
      ...
      debateStatistics: {
        activeDebates: activeDebates.size,
        totalMessages: Array.from(activeDebates.values()).reduce(
          (sum, debate) => sum + (debate.messageCount || 0),
          0,
        ),
  ...
  try {
    ...
    for (let attempt = 1; attempt <= 3 && !redisDisconnected; attempt++) {
      try {
        ...
        console.warn(
          `⚠️ Redis disconnect attempt ${attempt} failed:`,
          e.message,
        );
...
app.get("/api/contest/evaluation-summary", async (req, res) => {
  try {
    const [liveMetrics, optimizationMetrics, factCheckAnalytics] =
      await Promise.all([
        getLiveContestMetrics(),
        getOptimizationMetrics(),
        getFactCheckAnalytics(),
      ...
      categories: {
        redisInnovation: {
          ...
          strengths: [
            ...
            liveMetrics.multiModal?.advanced?.semanticCache?.active
              ? "Semantic caching implemented"
              : "Basic caching only",
            liveMetrics.innovation?.features?.intelligentAgents
              ? "AI agent intelligence"
              : "Standard agents",
            liveMetrics.innovation?.features?.realTimeOptimization
        ...
        technicalImplementation: {
          ...
          strengths: [
            ...
            optimizationMetrics.optimization?.status === "active"
              ...
              : "No optimization",
            liveMetrics.redis?.connectionHealthy
        ...
        realWorldImpact: {
          ...
          strengths: [
            ...
            liveMetrics.business?.costEfficiency?.semanticCacheSavings > 0
              ? "Cost savings achieved"
              : "No cost optimization",
          ],
        },
      },
      recommendations: generateContestRecommendations(
        liveMetrics,
        optimizationMetrics,
        factCheckAnalytics,
      ),
...
function generateContestRecommendations(
  liveMetrics,
  optimizationMetrics,
  factCheckAnalytics,
) {
  ...
  if ((liveMetrics.multiModal?.totalModulesActive || 0) < 4) {
    recommendations.push({
      ...
      priority: "high",
      recommendation:
        "Implement all 4 Redis modules (JSON, Streams, TimeSeries, Vector) for maximum scoring",
  ...
  if (!liveMetrics.multiModal?.advanced?.semanticCache?.active) {
    recommendations.push({
      ...
      priority: "medium",
      recommendation:
        "Enable semantic caching for AI responses to demonstrate advanced Redis usage",
  ...
  if ((liveMetrics.performance?.responseTimes?.average || 1000) > 200) {
    recommendations.push({
      ...
      priority: "high",
      recommendation:
        "Optimize response times to under 200ms for better performance scoring",
  ...
  if ((liveMetrics.performance?.cachePerformance?.hitRate || 0) < 70) {
    recommendations.push({
      ...
      priority: "medium",
      recommendation:
        "Improve cache hit rate to over 70% through better caching strategies",
      impact: "Up to +5 points",
    });
  }
  ...
  if ((liveMetrics.business?.userEngagement?.totalDebates || 0) < 3) {
    recommendations.push({
      ...
      priority: "medium",
      recommendation:
        "Demonstrate multiple concurrent debates to show scalability",
  ...
  if ((factCheckAnalytics.avgConfidence || 0) < 0.8) {
    recommendations.push({
      ...
      priority: "low",
      recommendation:
        "Enhance fact-checking accuracy to increase average confidence above 80%",
...
function determineOverallHealth(
  ...
  if ((liveMetrics.performance?.responseTimes?.average || 1000) < 300)
    healthScore += 25;
...
app.post("/api/agent/:agentId/intelligent-message", async (req, res) => {
  try {
    ...
    const result = await generateIntelligentMessage(
      agentId,
      debateId,
      topic,
      conversationHistory || [],
...
app.post("/api/demo/redis-matrix", async (req, res) => {
  try {
    console.log(
      "🎯 Redis Matrix demo triggered - Broadcasting all module operations",
    );
...
app.get("/api/analytics/contest-metrics", async (req, res) => {
  try {
    ...
    const cacheMetrics = await client.json
      .get("cache:metrics")
      .catch(() => null);
    ...
    const debateStats = {
      ...
      avgMessagesPerDebate:
        debateMetrics.debatesStarted > 0
          ? Math.round(
              debateMetrics.messagesGenerated / debateMetrics.debatesStarted,
            )
          : 0,
    };
...
app.get("/api/showcase/redis-modules", async (req, res) => {
  try {
    ...
    // RedisJSON showcase
    const jsonDemo = await client.json
      .get("agent:senatorbot:profile")
      .catch(() => null);
    showcase.redisJSON = {
    ...
    let streamDemo = null;
    if (latestStream) {
      streamDemo = await client
        .xRevRange(latestStream, "+", "-", { COUNT: 3 })
        .catch(() => null);
    }
    ...
    if (latestTimeseries) {
      timeseriesDemo = await client.ts
        .range(latestTimeseries, "-", "+", { COUNT: 5 })
        .catch(() => null);
    }
...
async function runMultiModalDemo(agents, duration) {
  ...
  try {
    ...
    for (const agentId of agents) {
      ...
      if (profile) {
        ...
        results.operations.push(
          `JSON.SET agent:${agentId}:profile demo_timestamp`,
        );
      }
    }
    ...
    if (factKeys.length > 0) {
      ...
      results.operations.push(
        `HGET ${randomFact} content: ${factContent?.substring(0, 50)}...`,
      );
    }
...
async function runPerformanceStressTest(duration) {
  ...
  results.operationsPerSecond = Math.round(
    (results.operations / results.duration) * 1000,
  );
...
async function runCacheEfficiencyDemo(agents, duration) {
  ...
  try {
    ...
    const cacheMetrics = await client.json
      .get("cache:metrics")
      .catch(() => null);
    results.efficiency = cacheMetrics || {
      hit_ratio: 0,
